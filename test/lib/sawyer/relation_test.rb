...
describe Sawyer::Relation do
  ...
  it "builds rels from hash index" do
    index = {
      'self' => {:href => '/users/1'}
    }
  ...
  it "relation api calls with uri tempate" do
    agent = Sawyer::Agent.for "http://foo.com/a" do |conn|
      conn.builder.handlers.delete(Faraday::Adapter::NetHttp)
      conn.adapter :test do |stubs|
  ...
  it "allows all methods when not in strict mode" do
